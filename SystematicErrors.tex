...
We used the data from st\_ssdmb stream (VPDMB-5-ssd trigger) from the same runs as the Roman Pot data used in our physics analyses. The HFT-tagged tracks were selected as the primary tracks passing the quality cuts \ref{sec:TpcQualityCuts} (only the TPC hits were counted). They were required to contain hits in two HFT layers: IST ans SST, which vastly reduced probability to select an off-time track in TPC (PXL was not used in reconstrucion due to problems in firmware). As shown in Fig.~\ref{fig:zVtxHFT}, the $z_{\text{vtx}}$ coverage of the HFT-tagges tracks is limited to about $\pm20$~cm, but we accept such limitation.% as we know that the TOF efficiency in the kinematic range of our measurements (Sec.~\ref{sec:TpcKinematicCuts}) 
\begin{itemize}
...
\parbox{0.65\textwidth}{ 
  \centering
		\begin{minipage}[t][0.64\linewidth][t]{\linewidth}\vspace{33pt}
