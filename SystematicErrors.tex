...
Systematic uncertainty of the TOF efficiency related to the accuracy of the TOF system simulation in STARsim and the TOF efficiency correction derived in Sec.~\ref{sec:tofAbsEffCorr} was estimated using the TPC tracks containing hits in HFT. In some STAR analyses the TOF hit reconstrucion and matching efficiency is determined from the data with the use BEMC: real (in-time) tracks are selected based on the fact that they match to BEMC cluster. If they do, the TOF hit reconstruction and matching efficiency is calculated as a ratio of TOF-matched tracks to all tracks. This solution is not fully safe, because the signal in the detector ``behind'' TOF (BEMC) ensures that nothing strange happens between the last hit of the track in TPC and TOF. Such efficiency may be higher than the efficiency without BEMC matching requirement.
