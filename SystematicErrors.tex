...
In order to find the systematic uncertainty,  the  TPC track selection efficiency was defined here as the~probabilty that global TPC track, passing  $d_0$ (\ref{sec:TpcQualityCuts}) qualty cut and compatibile with pion hypothesis $|n^{\sigma}_{\text{pion}}|<3$,  matched with TOF hits and true-level primary particle satisfies $N_{\text{hits}}^{\text{fit}}$ quality cut.  
