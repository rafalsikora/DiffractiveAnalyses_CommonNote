...
\begin{enumerate}
 ...
 \item Among primary TOF-matched TPC tracks preselected in \#3, one was set as a tag. In case only one track passed preselection, steps \#5-\#6 were executed only with this single track being a tag. Otherwise steps \#5-\#6 were repeated for every preselected track set as a tag.\\[-18pt]
...
w=\underbrace{\left(\varepsilon_{vtx}^{\text{nTOF=2}}(\Delta z_{0})\right)^{-1}}_{\text{2 TOF-matched prim. trks}}~~\text{or}~~~\underbrace{\left(\varepsilon_{vtx}^{\text{nTOF=1}}(d_{0})\times (\varepsilon_{vtx}^{\text{no-TOF}}(\vec{\text{DCA}})\right)^{-1}}_{\text{1 TOF-matched prim. trk}}
\end{equation}
